"""Re-evaluate preserved Cryptova-Raw argmax predictions.

No model is loaded. The preserved ``y_pred_argmax`` column is evaluated with
neither confidence thresholding nor the funding-rate/std_24h risk filter.
"""
...
MODEL = "cryptova_raw"
MODEL_VERSION = "original12_fusion_argmax_v1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-name", default="cryptova_raw_re_evaluation")
    return parser.parse_args()


def raw_frame(source_path: Path, metadata_path: Path, rolling: int) -> pd.DataFrame:
    frame = load_and_validate_source(source_path, metadata_path, rolling)
    frame["model"] = MODEL
    frame["model_version"] = MODEL_VERSION
    frame["y_pred"] = frame["base_y_pred_argmax"].astype("int64")
    return frame
...
def main() -> None:
    ...
    try:
        for rolling in (1, 2, 3):
            ...
            frame = raw_frame(source_path, metadata_path, rolling)
        ...
        manifest = {
            ...
            "final_prediction_column": "y_pred_argmax",
            "definition": "Fusion model argmax; no confidence threshold and no risk filter",
