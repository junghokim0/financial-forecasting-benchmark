"""Train and evaluate the direct SHORT/HOLD/LONG TimesNet classifier."""
...
MODEL_NAME = "timesnet_official_core_classifier"
...
def train_one_rolling(
    ...
    model = TimesNetClassifier(model_config).to(device)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the direct TimesNet classifier benchmark.")
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=BENCHMARK_ROOT / "outputs" / "timesnet_classifier"
    )
...
def main() -> None:
    ...
    if args.index_only:
        print(f"Rebuilt TimesNet classifier run index: {rebuild_run_index(args.output_dir, device)}")
        return
    ...
    print(f"Saved TimesNet classifier run index: {rebuild_run_index(args.output_dir, device)}")
