...
이 연구에서 내가 가장 먼저 확인하고 싶었던 것은 Cryptova가 기존 모델과 Foundation model
사이에서도 경쟁력을 가질 수 있는지였다. Connected OOS 결과를 보면, 적어도 현재 실험 범위에서는
그 가능성을 확인했다고 판단한다. Cryptova-Raw는 Macro F1 `0.381875`와 Balanced Accuracy
`0.393802`로 비교 모델 중 가장 높은 신호 분류 성능을 기록했다. Confidence와 Risk Filter를
적용하기 전에도 세 신호를 구분하는 모델 자체의 성능이 비교적 높았다는 점은 의미가 있다.

실제 운용 형태에 해당하는 Cryptova-Full은 비용 반영 수익률 `+27.46%`와 Sharpe-like
`+1.143`을 기록해 거래성과에서도 가장 앞섰다. 그러나 이 수익률만으로 안정적인 모델이
완성됐다고 보기는 어렵다. Full의 수익이 Rolling 3에 크게 의존했고 Rolling 2에서는 손실이
발생했기 때문이다. 따라서 나는 이번 결과를 **Cryptova의 경쟁 가능성은 확인했지만, 시장이
달라져도 성과가 유지되는 안정성까지 입증한 것은 아니다**라고 해석한다.

### 7.4 어떤 시장 regime에서 각 모델이 강한가?

전체 평균만 비교하면 각 모델이 어떤 시장에서 강하고 약한지 놓칠 수 있다고 생각해 Regime별
결과를 따로 확인했다. 수익률 수치 예측에서는 Ridge-Flat이 단기 하락·중립과 고·저변동성에서
가장 낮은 RMSE를 기록했다. 반면 Chronos-2가 가장 낮은 구간은 단기 상승뿐이었다. 이 결과를
보면서 복잡한 모델이 모든 시장 상태에서 항상 더 정확한 것은 아니라는 점을 확인할 수 있었다.
Regime별 RMSE의 최고 모델은 학습된 모델끼리 비교했으며, Zero-return baseline은 이 순위에서
제외했다.

신호 분류에서는 Cryptova-Raw가 단기 하락에서, Cryptova-Base가 고변동성에서 가장 높았다.
TimesNet Classifier는 단기 상승·중립·저변동성에서 가장 높은 Macro F1을 기록했다. 방향과
변동성을 결합한 결과는 차이를 더 분명하게 보여줬다. 고변동성 세 구간의 최고 모델은 모두
Cryptova 계열이었고, 저변동성 세 구간에서는 모두 TimesNet Classifier가 가장 높았다.

내가 특히 주목한 것은 실제 거래성과의 차이다. Cryptova-Full은 고변동성에서 `+48.74%`를
기록했지만 저변동성에서는 `-14.31%`였다. 따라서 현재 Cryptova의 강점은 모든 시장에서 고르게
작동한다는 데 있기보다 **하락·고변동성 환경에서 신호를 처리하고 고변동성 구간의 거래기회를
선별하는 능력**에 있다고 본다. 반대로 저변동성에서 불필요한 진입을 줄이고 Rolling별 성과
편차를 낮추는 일은 다음 개선 과제로 남아 있다.

### 7.5 예측 오차와 투자성과를 어떻게 볼 것인가?

이번 실험을 진행하면서 나는 **MSE를 낮추는 것만으로 좋은 투자 모델이 완성되는 것은
아니라는 점**을 분명히 느꼈다. 그렇다고 MSE가 중요하지 않다는 뜻은 아니다. MSE와 RMSE는
예측값이 실제 수익률에서 얼마나 벗어났는지 보여주는 중요한 지표지만, 그 개선이 실제
투자성과로 이어지는지까지 확인해야 한다. 금융
데이터에서는 수익률이 0% 근처에 많이 모여 있기 때문에, 거의 모든 값을 0%로 예측해도 낮은
오차를 얻을 수 있다. 이 경우 숫자상의 오차는 작아도 실제 매수·매도 판단에는 활용하기 어렵다.

이번 결과에서도 이 차이가 나타났다. Zero-return baseline은 RMSE `0.022948`로 모든 학습
모델보다 낮았지만 거래 신호를 만들지 않는다. 학습 모델 중 RMSE가 가장 낮았던 Ridge-Flat도
비용 반영 수익률은 약 `-0.005%`로 실질적인 수익을 만들지 못했다. 반면 Cryptova-Full은
Regression RMSE를 직접 최적화하는 모델이 아니지만 Connected OOS에서 `+27.46%`의 비용 반영
수익률과 Sharpe-like `+1.143`을 기록했다.

그래서 나는 예측 오차와 투자성과를 서로 대체하는 지표가 아니라 **서로 다른 질문에 답하는 두
평가축**으로 본다. MSE·RMSE는 미래 수익률 값을 얼마나 정확히 예측했는지 평가하고, 비용 반영
수익률·MDD·Sharpe-like·거래 수와 Regime별 안정성은 그 예측이 실제 투자 의사결정에 도움이
됐는지 평가한다. 최종 모델의 가치는 낮은 오차 하나로 판단하기보다, Validation에서 규칙을
고정한 뒤 Test에서 확인된 투자성과와 위험을 함께 보고 판단해야 한다.
