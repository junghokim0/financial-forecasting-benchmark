...
- 목적: 모든 비교 모델에 적용할 공통 실험 조건을 고정하고, 모델별 설정과 결과를 같은 형식으로 누적하여 최종 연구 결론을 도출한다.
- 공통 설정: 원칙적으로 각 모델은 관측 시점 기준 과거 72시간의 정보를 바탕으로 미래 24시간 수익률 또는 SHORT/HOLD/LONG 신호를 예측한다. 단, TimesFM 2.5는 모델의 32-step patch 제약으로 공통 72시간 범위 내 최근 64시간을 입력받아 미래 24시간을 예측한다.
...
주 비교는 각 모델을 실제 사용 방식으로 구성한 뒤 동일한 시점, target 및 evaluator에서 평가하는 **end-to-end system comparison**이다.
...
이 표만으로 모델의 순위를 하나로 정하기는 어려웠다. TimesNet이 그 이유를 가장 잘 보여준다.
...
내 결론은 **조건부로 경쟁력이 있다**는 것이다. Raw 상태에서도 Macro F1 `0.381875`와
Balanced Accuracy `0.393802`로 가장 높은 분류 성능을 냈다는 점에서, 비교 결과를 후처리의
효과만으로 설명할 수는 없다. 반대로 실제 투자성과는 Full에서 가장 좋았다. 비용을 반영하고도
수익률 `+27.46%`, Sharpe-like `+1.143`을 기록했다.

다만 여기서 바로 “안정적인 수익 모델이 완성됐다”고 결론 내리지는 않으려 한다. Rolling 2의
손실과 Rolling 3에 집중된 이익이 남아 있기 때문이다. 지금 확인한 것은 Cryptova가 비교
모델들과 겨뤄 볼 만한 신호 분류력과 수익 가능성을 갖고 있다는 점이다. 시장이 바뀌어도 같은
성과를 낼 수 있는지는 아직 다음 검증의 몫이다.

### 7.4 어떤 시장 regime에서 각 모델이 강한가?

전체 평균에서는 보이지 않던 차이가 Regime을 나누자 드러났다. 수익률 값을 맞히는 문제에서는
Ridge-Flat이 단기 하락·중립과 고·저변동성에서 가장 낮은 RMSE를 보였다. Chronos-2가 앞선
구간은 단기 상승뿐이었다. 복잡한 구조가 모든 상황에서 유리할 것이라는 예상과는 다른
결과였다. 이 비교는 학습된 모델만을 대상으로 했기 때문에 Zero-return baseline은 Regime별
순위에서 뺐다.

신호 분류의 경계는 변동성이었다. 고변동성 세 조합에서는 Cryptova 계열이 모두 1위였고,
저변동성 세 조합에서는 TimesNet Classifier가 모두 앞섰다. Cryptova-Full의 거래 결과도 같은
방향을 가리킨다. 고변동성에서는 `+48.74%`였지만 저변동성에서는 `-14.31%`였다.

이 수치를 보고 Cryptova를 모든 시장에 고르게 강한 모델로 보기는 어려웠다. 오히려 하락과
고변동성이 겹치는 상황에서 강점이 선명해지는 모델에 가깝다. 그렇다면 다음 개선 방향도
분명하다. 저변동성에서 필요 없는 진입을 줄이고, 어느 Rolling에서도 비슷한 수준의 성과가
나오도록 만드는 일이다.

### 7.5 예측 오차와 투자성과를 어떻게 볼 것인가?

결과를 비교하는 동안 가장 크게 바뀐 생각은 “오차가 가장 낮은 모델이 투자에도 가장 좋은가”에
대한 답이었다. 이번 실험에서는 그렇지 않았다. 아무 학습도 하지 않고 수익률을 늘 0%로 두는
Zero-return baseline의 RMSE가 `0.022948`로 가장 낮았지만, 이 예측으로는 거래 신호를 만들 수
없다. Ridge-Flat도 같은 차이를 보여줬다. Regression용으로 선택한 variant는 학습 모델 중 가장
낮은 RMSE를 기록했지만, 별도로 Classification 기준으로 선택한 variant의 비용 반영 수익률은
약 `-0.005%`였다.

이 경험 때문에 MSE를 덜 중요하게 보게 된 것은 아니다. MSE와 RMSE는 수익률 값을 얼마나
가깝게 맞혔는지 알려준다. 다만 실제 투자를 평가하려면 질문을 하나 더 해야 한다. 그 예측을
따라 거래했을 때 비용을 내고도 수익이 남았는가? Cryptova-Full은 Regression RMSE를 직접
최적화하지 않았지만 Connected OOS 수익률 `+27.46%`와 Sharpe-like `+1.143`을 기록했다.

그래서 예측 오차와 투자성과를 하나의 순위로 합치지 않았다. 전자는 MSE·RMSE로 확인하고,
후자는 비용 반영 수익률, MDD, Sharpe-like, 거래 수, Regime별 안정성으로 살펴봤다. 모델을
최종적으로 판단할 때는 Validation에서 규칙을 먼저 고정한 다음, Test에서 수익과 위험이 함께
어떻게 나타났는지를 보는 편이 이 연구의 목적에 더 맞다고 생각한다.

### 7.6 결과 해석 시 제한사항

이 결론이 모델 구조만의 우열을 뜻하는 것은 아니다. TimesFM은 close-only 입력을 사용했고,
Chronos-2는 Chart covariate를 함께 받았다. Cryptova에는 News까지 들어간다. 입력 modality와
출력 방식이 서로 다르므로, 현재 비교는 동일한 입력을 넣은 architecture-only 실험보다 각
시스템을 정해진 조건에서 끝까지 평가한 end-to-end 비교에 가깝다. Foundation model이 이미
대규모 외부 데이터로 사전학습됐다는 차이도 해석에 포함해야 한다.

검증 규모에도 한계가 있다. Rolling은 세 구간이고 주요 neural model은 Seed 42 한 번으로
평가했다. 거래 수도 통계적 확신을 주기에 충분한지 더 살펴볼 필요가 있다. 무엇보다 연결 Test
결과를 본 뒤 규칙을 다시 고친다면 그 Test는 더 이상 완전히 새로운 데이터라고 보기 어렵다.
그 경우에는 변경한 규칙을 고정하고 별도의 untouched holdout에서 한 번 더 확인해야 한다.
