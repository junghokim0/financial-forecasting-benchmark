"""Train and evaluate the many-to-one LSTM on benchmark rolling splits."""
...
from lstm_model import LSTMConfig, ManyToOneLSTM, count_trainable_parameters  # noqa: E402


MODEL_NAME = "lstm_many_to_one"
MODEL_VERSION = "lstm_many_to_one_v1"
...
def save_variant(
    ...
    model = ManyToOneLSTM(model_config).to(device)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the many-to-one LSTM benchmark.")
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=BENCHMARK_ROOT / "outputs" / "lstm_regression"
    )
...
def main() -> None:
    ...
    if args.index_only:
        ...
        print(f"Rebuilt LSTM run index: {index_path}")
        return
    ...
    index_path = rebuild_run_index(args.output_dir, device)
    print(f"Saved LSTM run index: {index_path}")
