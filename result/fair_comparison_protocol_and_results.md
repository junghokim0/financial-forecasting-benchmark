...
### 7.6 예측 오차와 투자성과를 어떻게 볼 것인가?

이용재 교수님의 조언을 나는 MSE를 무시하라는 뜻이 아니라, **MSE를 낮추는 것만으로 연구를
끝내지 말고 그 개선이 실제 투자성과로 이어지는지 확인해야 한다**는 뜻으로 받아들였다. MSE와
RMSE는 예측값이 실제 수익률에서 얼마나 벗어났는지 보여주는 중요한 지표다. 그러나 금융
